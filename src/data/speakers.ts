import type { Speaker } from "../types";

export const speakerOlga = 	{
	name: 'Olga Tohabanda',
	image: 'assets/img/speakers/Olga_Tohabanda.png',
	role: 'UX Researcher',
	bio: '¡Hola! mi nombre es Olga Tohabanda Duchi, fundadora de la comunidad de Mujeres HerUX. Profesional en Diseño Web y Aplicaciones Multimedia, pero amante del UX Research(Investigación de experiencia del usuario). Brindo consultorías de Investigación y Experiencia del Usuario a empresas IT y emprendedores cuya visión es mejorar sus productos o servicios desde la UX. Mi habilidad para gestionar me permite reclutar y entrevistar a todos los involucrados en el proyecto mediante la metodología Design Thinking. Conozco herramientas como: Figma, Miro, Maze, y además cuento con más de 3 años de experiencia en el área de Diseño&Investigación.',
	inspiringPhrase: '<<No es cómo empiezas algo, sino cómo lo terminas>> - Olga Tohabanda.',
	linkedin: 'https://www.linkedin.com/in/olgatohabandaduchi',
};

export const speakerZully = {
  name: 'Zully Moncayo',
  image: 'assets/img/speakers/Zully_Moncayo.jpeg',
  role: 'Tech Manager',
  bio: 'Soy Zully Moncayo, Ingeniera en Software con un máster en seguridad de datos, me apasiona la tecnología y el mundo de los pagos. Con más de 8 años de experiencia, he desarrollado soluciones de pago para P.O.S y he integrado sistemas con adquirentes en toda Latinoamérica, incluyendo Ecuador, Colombia, Argentina y México. Actualmente, como Tech Manager en una startup, trabajo de forma remota y estoy construyendo mi marca personal compartiendo contenido en redes sociales.',
	inspiringPhrase: '<<Somos el promedio de las cinco personas que nos rodean>> - Jim Rohn. Esta frase resume la idea de que las personas con las que nos asociamos tienen un impacto significativo en nuestra vida y en nuestra forma de pensar, sentir y comportarnos. ',
  instagram: 'https://www.instagram.com/zullymoncayo/',
  linkedin: 'https://www.linkedin.com/in/zully-moncayo',
};

export const speakerMartha = {
  name: 'Martha Tomalá',
  image: 'assets/img/speakers/Martha_Tomala.jpeg',
  role: 'Analista de datos',
  bio: 'Soy ingeniera estadística y actualmente estoy finalizando un máster en Big Data y Ciencia de Datos. Actualmente desempeño el rol de analista de datos en una institución financiera. Tengo experiencia en la aplicación de técnicas estadísticas y de modelado para resolver problemas en diversas áreas, incluyendo la industria, la salud y las ciencias sociales.',
	inspiringPhrase: '<<La principal ventaja de los datos es que te dicen algo sobre el mundo que no sabías antes>> - Hilary Mason, Científica de datos y Fundadora, Fast Forward Labs',
  linkedin: 'https://www.linkedin.com/in/martha-julissa-tomal%C3%A1-m%C3%A9ndez-81a8111a3',
};

export const speakerSofia = {
  name: 'Sofía España',
  image: 'assets/img/speakers/Sofia_Espana.jpg',
  role: '',
  bio: 'Ing. en mecatrónica con experiencia en distintas áreas del mundo STEM',
	inspiringPhrase: '<<El mundo es tan grande como tu mente te lo permita>>',
  linkedin: 'https://www.linkedin.com/in/sofia604/',
};

export const speakerAndrea = {
  name: 'Andrea Simbaña',
  image: 'assets/img/speakers/Andrea_Simbana.png',
  role: '',
  bio: 'Andrea Simbaña, es ingeniera electrónica egresada de la UPS Quito, actualmente esta cursando una maestría en Educación con énfasis en TICs. Además de ser maestra de robótica en primaria y coach de competencia, es miembro voluntario del IEEE desde 2021. En el IEEE, ha desempeñado roles de secretaria, community manager, y actualmente forma parte de IEEE Students Sección Ecuador.',
	inspiringPhrase: '<<La exploración espacial es un recordatorio diario de que somos capaces de más de lo que creemos posible, y que la curiosidad y la colaboración pueden llevarnos más allá de lo que imaginamos>> - Kellie Gerardi',
  linkedin: 'https://www.linkedin.com/in/andrea-simba%C3%B1a-778100214/',
};

export const speakerValeria = {
  name: 'Valeria Barzola',
  image: 'assets/img/speakers/Valeria_Barzola.jpeg',
  role: 'Líder técnico',
  bio: 'Ingeniera en Ciencias de la Computación, apasionada por las tecnologías emergentes y su potencial para revolucionar industrias. Poseo experiencia en desarrollo web y móvil, así como en la implementación de soluciones tecnológicas innovadoras. Me dedico a investigar y adoptar las últimas tendencias en tecnología, utilizando este conocimiento para resolver problemas complejos y generar un impacto significativo en los proyectos que lidero.',
	inspiringPhrase: '<<In the future, there will be no female leaders. There will just be leaders.>> - Sheryl Sandberg',
  linkedin: 'https://www.linkedin.com/in/valeria-barzola/',
};

export const speakers: Array<Speaker> = [
  speakerOlga,
  speakerZully,
  speakerMartha,
  speakerSofia,
  speakerAndrea,
  speakerValeria
];
